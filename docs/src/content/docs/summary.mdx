...
```
Input text: "I like transformers"
Tokens:     [<BOS>, I, like, transformers, <EOS>]
Token IDs:  [1, 3, 4, 5, 2]
...
Result: [5, 16] matrix of embedded tokens
```
...
Result: [5, 16] final hidden states
```
...
Shape: [4, 6] (4 prediction positions, 6 vocab tokens)
```

**9. Output Layer Gradients** → Language modeling head
```
∂L/∂W_lm: [6, 16] - how to change the output projection
∂L/∂hidden: [5, 16] - gradients flowing to layer norm
```
...
```
∂L/∂E_token: [6, 16] - how to change token embeddings
∂L/∂E_pos: [5, 16] - how to change position embeddings
```
...
```
Applied AdamW optimizer to all 2,456 parameters:
...
Parameter updates:          2,456 updates
─────────────────────────────────────────────
Total:                      ~4,800 operations
```

That's what happens for **one training step** on **one example** with **5 tokens**.

### Parameter Breakdown

```
Token embeddings:         6 tokens × 16D = 96 params
Position embeddings:      5 positions × 16D = 80 params
...
LM head:                    6 × 16 = 96 params
─────────────────────────────────────────────
Total:                      2,456 parameters
```

All 2,456 received gradients. All 2,456 were updated.
...
### Linear Algebra
- [Matrix multiplication](https://en.wikipedia.org/wiki/Matrix_multiplication) (hundreds of times)
- [Transpose operations](https://en.wikipedia.org/wiki/Transpose)
- [Dimension tracking](https://en.wikipedia.org/wiki/Dimension_(vector_space))
- [Batch operations](https://en.wikipedia.org/wiki/Batch_processing)

### Calculus
- [Partial derivatives](https://en.wikipedia.org/wiki/Partial_derivative)
- [Chain rule](https://en.wikipedia.org/wiki/Chain_rule) (repeatedly)
- [Jacobian matrices](https://en.wikipedia.org/wiki/Jacobian_matrix_and_determinant) (for softmax, layer norm)
- [Gradient computation](https://en.wikipedia.org/wiki/Gradient)
- [Backpropagation](https://en.wikipedia.org/wiki/Backpropagation)

### Optimization Theory
- [Gradient descent](https://en.wikipedia.org/wiki/Gradient_descent)
- [Adaptive learning rates](https://en.wikipedia.org/wiki/Learning_rate)
- [Momentum](https://en.wikipedia.org/wiki/Stochastic_gradient_descent#Momentum)
- [Weight decay](https://en.wikipedia.org/wiki/Regularization_(mathematics))
- [Bias correction](https://en.wikipedia.org/wiki/Stochastic_gradient_descent#Adam)

### Numerical Methods
- [Softmax](https://en.wikipedia.org/wiki/Softmax_function) (for numerical stability)
- [Layer normalization](https://en.wikipedia.org/wiki/Batch_normalization#Layer_normalization)
- [GELU approximations](https://en.wikipedia.org/wiki/Activation_function#Gaussian_error_linear_unit_(GELU))
- [Float32 arithmetic](https://en.wikipedia.org/wiki/Single-precision_floating-point_format)
...
**Larger vocabulary**: We had 6 tokens. Real models have 50,000+ tokens.
...
# Our input
tokens = torch.tensor([[1, 3, 4, 5, 2]])  # [batch=1, seq_len=5]

# Targets (shifted by one)
targets = torch.tensor([[3, 4, 5, 2]])  # Predict next token
...
# Compute loss
loss = criterion(logits.view(-1, 6), targets.view(-1))  # ✓ Step 8

# Backward pass
loss.backward()  # ✓ Steps 9-12 (all gradients computed automatically!)

# Optimization step
optimizer.step()  # ✓ Step 13 (all 2,456 parameters updated)
...
7. ✅ Stored all 2,456 gradients in `.grad` attributes
...
6. ✅ Updated all 2,456 parameters
